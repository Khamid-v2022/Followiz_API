...
class Database{
    ...
    public function get_all_records(){
        ...
        if(isset($_POST['daterange'])){
                // $this->date_range = implode(' - ',$_POST['daterange']);
            ...
            $stm = $this->conn->query("SELECT * FROM invoices WHERE CONVERT(DATE, created_at, 103) >= $start_date AND CONVERT(DATE, fromDate, 103) < $end_date");
        // $stm = $this->conn->query("select * from invoices where created_at Like '%$start_date%' ");
         ...
         $this->number_of_records = $stm->rowCount();
          // return $this->number_of_records;
             ...
             $stm = $this->conn->query("select * from invoices LIMIT $this->start_from, $this->per_page_record");
        }
    ...
    public function store_details($data){
        ...
        try {
            ...
            $sql = "INSERT INTO invoices (user_id, user_name, invoice_name)
            VALUES ('".$data["user_id"]."', '".$data["user_name"]."', '".$data["invoice_name"]."')";
